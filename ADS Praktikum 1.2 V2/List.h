...
void List<T>::insertFront(T key)
{
    ...
    */
    ...
    Node<T>* neuer_knoten = new Node<T>;  //knoten wird erstellt
    neuer_knoten->key = key;

    if (head_tail->next == head_tail) { //wenn liste leer
    ...
    list_size++;    //liste wird vergrößert
...
void List<T>::insertFront(List<T>& _List)
{
    ...
    */
    ...
    if (head_tail->next == head_tail) {
        ...
        //_list wird geschlossen
        _List.head_tail->next = _List.head_tail;
        _List.head_tail->prev = _List.head_tail;
        //Liste A wird um die größe von _List vergrößert
        list_size += _List.list_size;
        //_List wird auf 0 gesetzt
        _List.list_size = 0;
    }
    //Wenn beide listen voll
    else {
...
bool List<T>::getFront(T& key)
{
    ...
    //key ist der wert des knotens, der gelöscht wird
    key = head_tail->next->key;

    //wenn liste 1 knoten (außer head_tail) hat
    if (head_tail->next == head_tail->prev) {

        head_tail->next->key = key;         //wasbruda?
...
bool List<T>::del(T key)
{
    ...
    Node* todelete = head_tail;

    while (todelete->next != head_tail && todelete->key != key) {

        todelete = todelete->next;
    }
        //wenn nur 1 knoten da ist
    if (head_tail->next == head_tail->prev) {

        //wenn 1 knoten und gesuchter knoten gefunden
        if (todelete->key = key) {
            delete todelete;
            head_tail->next = head_tail;
            head_tail->prev = head_tail;
        ...
        //wenn 1 knoten und gesuchter knoten nicht vorhanden
        else {
            return false;
        }
    }
...
*/
    Node<T>* ptr = head_tail;

    while (ptr->next != head_tail && ptr->key != key) {
...
bool List<T>::swap(T key1, T key2)
{
    ...
    while (ptr1 != head_tail && ptr1->key != key1) {
        ptr1 = ptr1->next;
    }

    Node<T>* tmp = ptr1;

    while (ptr2 != head_tail && ptr2->key != key2) {
        ptr2 = ptr2->next;
    }
    ...
    if (ptr1 != head_tail && ptr2 != head_tail) {
        ...
        Node<T>* temp_knoten1 = new Node<T>;

        temp_knoten1->next = ptr1->next;
        temp_knoten1->prev = ptr1->prev;

        Node<T>* temp_knoten2 = new Node<T>;

        temp_knoten2->next = ptr2->next;
        temp_knoten2->prev = ptr2->prev;

        // ersten Temp Knoten verketten
        ptr1->next->prev = temp_knoten1;
        ptr1->prev->next = temp_knoten1;
        temp_knoten1->next = ptr1->next;
        temp_knoten1->prev = ptr1->prev;

        // Zweiten temp Knoten verketten
        ptr2->prev->next = temp_knoten2;
        ptr2->next->prev = temp_knoten2;
        temp_knoten2->next = ptr2->next;
        temp_knoten2->prev = ptr2->prev;


        // Knoten Reihenfolge verändern
        ptr1->next = temp_knoten2->next;
        ptr1->prev = temp_knoten2->prev;
        temp_knoten2->prev->next = ptr1;
        temp_knoten2->next->prev = ptr1;

        ptr2->next = temp_knoten1->next;
        ptr2->prev = temp_knoten1->prev;
        temp_knoten1->next->prev = ptr2;
        temp_knoten1->prev->next = ptr2;
    ...
    */
    /*
//Finden der Knoten
    Node* pointer = head_tail;
    Node* p1 = nullptr;
    Node* p2 = nullptr;
    bool found = false;
    for (int i = 0; i < list_size + 1; i++) {
        if (pointer->key == key1)p1 = pointer;
        if (pointer->key == key2)p2 = pointer;
    ...
    //Neu Verketten der Knoten

    if (found) {

        Node* temp1 = new Node(0, p1->next, p1->prev);
        Node* temp2 = new Node(0, p2->next, p2->prev);

        //Temp Knoten verketten
        //T1
        p1->prev->next = temp1;
        temp1->next = p1->next;
        temp1->prev = p1->prev;
        ...
        temp2->prev = p2->prev;
        p2->next->prev = temp2;
        //Richtige Knoten wieder einbauen
        //P2
        temp1->prev->next = p2;
        p2->next = temp1->next;
        p2->prev = temp1->prev;
        ...
        p1->prev = temp2->prev;
        temp2->next->prev = p1;
        //Löschen der beiden Hilfsknoten
        delete temp1;
        delete temp2;
        return true;
    }
...
*/
    ...
    if (head_tail->next == head_tail) {
        ...
        return counter;	// Hier richtiges Ergebnis zurückgeben
    }
...
//        Node<T>* neuer_eintrag = new Node<T>(key, NULL, NULL);
//
//        head_tail->next = neuer_eintrag;
//        neuer_eintrag->prev = head_tail;
