...
int EdgeWeightedDigraph::degree(EdgeWeightedDigraph G, int v) {
	return G.getAdj(v).size();
}

int EdgeWeightedDigraph::maxDegree(EdgeWeightedDigraph G) {
	int max = 0;
...
double EdgeWeightedDigraph::averageDegree(EdgeWeightedDigraph G) {
	return static_cast<double>(2 * G.getE()) / static_cast<double>(G.getV());
}
